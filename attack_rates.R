...
ft <- 29   # 1 year follow-up time
empty_year_cohort <-  matrix (0, 12, ncol = ft) # extrapolate age-based attack rates out to 18 months, for 6-18 months project linear decline starting with AR at 6mo. and ending with AR at3mo.
...
age_AR_y <- c(age_AR, seq(age_AR[6], age_AR[3], length = 12))
...
cal_AR_y <- c(cal_AR, cal_AR, cal_AR[1:5])
...
AR_cases_year <- c(age_dat$cases, seq(age_dat$cases[6], age_dat$cases[3], length = 12))
AR_ft_year <- c(age_dat$follow_up, seq(age_dat$follow_up[6], age_dat$follow_up[3], length =12))
