...
ft <- 35   # 2 years follow-up time
empty_year_cohort <-  matrix (0, 12, ncol = ft) # extrapolate age-based attack rates out to 24 months, for 6-24 months project linear decline starting with AR at 6mo. and ending with AR at3mo.
...
### create AR matrices for birth cohorts followed for two years
age_AR_y <- c(age_AR, seq(age_AR[6], age_AR[3], length = 18))
...
cal_AR_y <- c(cal_AR, cal_AR, cal_AR[1:11])
...
AR_cases_year <- c(age_dat$cases, seq(age_dat$cases[6], age_dat$cases[3], length = 18))
AR_ft_year <- c(age_dat$follow_up, seq(age_dat$follow_up[6], age_dat$follow_up[3], length =18))
